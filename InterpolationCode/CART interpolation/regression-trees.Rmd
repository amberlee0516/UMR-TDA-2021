...
water_data$FLDNUM <- as.character(water$FLDNUM)
water_data$STRATUM <- as.character(water$STRATUM)
water_data$season <- as.character(water$season)
...
make_trees <-function(target_var, df) {
  ...
  model_controls <- rpart.control(cp = .1)
...
## append the dataframes with better names
