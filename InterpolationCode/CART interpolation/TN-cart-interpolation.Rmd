...
data <- read.csv(file = "../../LTRM data/water_data_qfneg.csv")
...
set.seed(4321)
tr.TN_rpart1 <- rpart(TN ~ pred_col,
                    data = data_train,
                    method = "anova",
                    control = (maxdepth = 20))
fancyRpartPlot(tr.TN_rpart, main = "TN prediction for all years")
...
# Split by year and season
make_year_tuples <- functon(index, year_partition) {
  return(c(year_partition[index], year_partition[index + 1]))
}
...
# Get the RMSE and correlation coefficient for the test data
RMSE(data_train$TN, data_train$PREDICTED, na.rm = TRUE)

cor(data_test$TN, data_test$PREDICTED, use = "complete.obs")
